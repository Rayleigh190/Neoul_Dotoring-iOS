...
//                        HomeNetworkManager.shared.authenticator.handleAuthenticationError(urlRequest: urlToCall.urlRequest!, response: HTTPURLResponse(url: (urlToCall.urlRequest?.url)!, statusCode: 401, httpVersion: "HTTP/1.1", headerFields: nil)!, error: AuthError.tokenExpired)
