...
//                        KeyChain.create(key: KeyChainKey.refreshToken, token: parsedRefreshToken)
    ...
    class func getMockLogin(userID: String, userPW: String, setAutoLogin: Bool, _ completion: @escaping (LoginAPIResponse?, Error?) -> Void) {
        // 사용자가 입력한 아이디와 비밀번호를 받습니다.
        let urlToCall = BaseRouter.userLogin(userID: userID, userPW: userPW)
        
        // JSON 파일에서 Mock 데이터와 HTTP 상태 코드 설정
        MockAPINetworkManager.shared.setMockDataAndStatusCode(fromJSONFile: "getLoginStandby", statusCode: 200)
